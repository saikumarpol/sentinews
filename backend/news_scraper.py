...
async def _fetch_category_news(query: str, category: str) -> List[Dict[str, Any]]:
    ...
    if NEWSAPI_KEY:
        try:
            ...
            articles = resp.json().get("articles", [])
            news = [
                _make_item(i, (a.get("source") or {}).get("name") or "NewsAPI", a.get("title") or "", a.get("url") or "", _parse_pub_date(a.get("publishedAt")), a.get("description") or "", category)
                for i, a in enumerate(articles) if a.get("title")
            ]
            from ai_processor import enrich_news_batch
            news = enrich_news_batch(news)
        except Exception as exc:
            logger.error(f"{category} NewsAPI failed: {exc}")
    
    # Supplement with RSS
    for source_name, url, cat in RSS_FEEDS:
        if cat == category:
            # High priority for direct category match
            news.extend(_fetch_rss_sync(source_name, url, category))
        elif cat == "macro" or cat == "markets":
            # Lower priority: filter general news for keywords
            general_news = _fetch_rss_sync(source_name, url, category)
            keywords = query.lower().split(" or ")
            filtered = [
                n for n in general_news 
                if any(k.strip() in n["headline"].lower() or k.strip() in n["summary"].lower() for k in keywords)
            ]
            news.extend(filtered)
    
    # Final sort: most relevant (keyword matches) first
    keywords = query.lower().split(" or ")
    def relevance_score(n):
        count = sum(1 for k in keywords if k.strip() in n["headline"].lower())
        return count * 10 + (1 if any(k.strip() in n["summary"].lower() for k in keywords) else 0)
    
    news.sort(key=lambda x: (relevance_score(x), x["published_at"]), reverse=True)
    return news[:40]
...
async def get_commodities_dashboard(is_futures: bool = False) -> Dict[str, Any]:
    market_data = await commodities.fetch_market_dashboard(is_futures=is_futures)
    snapshot = {
...
async def get_indices_dashboard(is_futures: bool = False) -> Dict[str, Any]:
    market_data = await commodities.fetch_market_dashboard(is_futures=is_futures)
    snapshot = {
        "date":        market_data["last_updated"],
...
async def get_currencies_dashboard(is_futures: bool = False) -> Dict[str, Any]:
    market_data = await commodities.fetch_market_dashboard(is_futures=is_futures)
    snapshot = {
        "date":        market_data["last_updated"],
